...
const inputElement4 = document.getElementById('filepond-a4');
const pond_a4 = FilePond.create( inputElement4 );

function upload_to_lambda_a4(fieldName, file, metadata, load, error, progress, abort) {
  $('#result-a1').css('visibility', 'hidden');
  var formData = new FormData();
  formData.append(file.name, file)
  ...
  $.ajax({
      ...
      retryLimit : 3,
      method: 'POST',
      url: "https://j91tpbv5f3.execute-api.ap-south-1.amazonaws.com/dev/assignment4-facenet",
      data: formData,
      processData: false,
      contentType: false,
      mimeType: "multipart/form-data",
  ...
  query_data(max_retry_count, (status, response) => {
    console.log(status, response)
    $('#result-a4').css('visibility', 'visible');
    if(status == 'fail'){
      ++step_count;
      progress(true, 1, 1);
      $('#result-a4').attr('class', 'alert alert-danger');
      $('#result-a4-prediction').text('Operation failed, please retry')
      error('Failed after ' + max_retry_count.toString() + 'retries');
    } else if (status == 'retrying') {
      progress(true, ++step_count * step_size, 1);
      $('#result-a4').attr('class', 'alert alert-warning');
      $('#result-a4-prediction').text('Please wait ...')
    } else {
      progress(true, 1, 1);
      $('#result-a4').attr('class', 'alert alert-success');
      $('#result-a4-prediction').empty()
      response = JSON.parse(response)
      $('#result-a4-prediction').append('<div class="text-left">')
        Object.keys(response).forEach((key) => {
          $('#result-a4-prediction').append(
            '<p>' 
              + '<h5 class="text-uppercase">' + key + '</h5>' + response[key] 
          + '</p>');
      });
      $('#result-a4-prediction').append('</div>')
    }
  });
...
pond_a4.setOptions({
  ...
  maxFiles: 1,
  server: {
    process: upload_to_lambda_a4,
    revert: null
  }});

$('#result-a4').css('visibility', 'hidden');